...
function showFirstAndLast(arr) {
  const firstLetters = arr.map((firstL) => firstL[0]);
  const lastLetters = arr.map((lastL) => lastL[lastL.length - 1]);

  let firstAndLast = [];

  for (let i = 0; i < firstLetters.length; i++) {
    firstAndLast.push(firstLetters[i] + lastLetters[i]);
  }
  return firstAndLast;
}
/*
...
function doubleOddNumbers(arr) {
  return arr
    .filter(function (findOdds) {
      if (findOdds % 2 !== 0) {
        return findOdds;
      }
    })
    .map(function (dblOdds) {
      return dblOdds * 2;
    });
}
